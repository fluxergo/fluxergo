...
import (
	...
	"github.com/fluxergo/fluxergo/internal/insecurerandstr"
)
...
type MemberChunkingManager interface {
	...
	// HandleChunk handles the fluxer.EventGuildMembersChunk event payloads from the discord Gateway.
	...
	RequestMembers(ctx context.Context, guildID snowflake.ID, userIDs ...snowflake.ID) ([]fluxer.Member, error)
	// RequestAllMembers requests all members from the given guildID.
	// Notice: This action requires the gateway.IntentGuildMembers.
	RequestAllMembers(ctx context.Context, guildID snowflake.ID) ([]fluxer.Member, error)
	// RequestMembersWithQuery requests members from the given guildID and query.
	// query : string the username starts with
	// Notice: This action requires the gateway.IntentGuildMembers.
	RequestMembersWithQuery(ctx context.Context, guildID snowflake.ID, query string, limit int) ([]fluxer.Member, error)
	...
	// Notice: This action requires the gateway.IntentGuildMembers.
	RequestMembersWithFilter(ctx context.Context, guildID snowflake.ID, memberFilterFunc func(member fluxer.Member) bool) ([]fluxer.Member, error)
	...
	// Notice: This action requires the gateway.IntentGuildMembers.
	RequestMembersChan(guildID snowflake.ID, userIDs ...snowflake.ID) (<-chan fluxer.Member, func(), error)
	// RequestAllMembersChan requests all members from the given guildID.
	// Returns a channel which will receive the members.
	// Returns a function which can be used to cancel the request and close the channel.
	// Notice: This action requires the gateway.IntentGuildMembers.
	RequestAllMembersChan(guildID snowflake.ID) (<-chan fluxer.Member, func(), error)
	// RequestMembersWithQueryChan requests members from the given guildID and query.
	// Returns a channel which will receive the members.
	// Returns a function which can be used to cancel the request and close the channel.
	// Notice: This action requires the gateway.IntentGuildMembers.
	RequestMembersWithQueryChan(guildID snowflake.ID, query string, limit int) (<-chan fluxer.Member, func(), error)
	...
	// Returns a function which can be used to cancel the request and close the channel.
	// Notice: This action requires the gateway.IntentGuildMembers.
	RequestMembersWithFilterChan(guildID snowflake.ID, memberFilterFunc func(member fluxer.Member) bool) (<-chan fluxer.Member, func(), error)
}
...
type chunkingRequest struct {
	...
	memberChan       chan<- fluxer.Member
	memberFilterFunc func(member fluxer.Member) bool
...
func (m *memberChunkingManagerImpl) requestGuildMembersChan(ctx context.Context, guildID snowflake.ID, query *string, limit *int, userIDs []snowflake.ID, memberFilterFunc func(member fluxer.Member) bool) (<-chan fluxer.Member, func(), error) {
	...
	memberChan := make(chan fluxer.Member)
...
func (m *memberChunkingManagerImpl) RequestMembers(ctx context.Context, guildID snowflake.ID, userIDs ...snowflake.ID) ([]fluxer.Member, error) {
	if len(userIDs) == 0 {
...
func (m *memberChunkingManagerImpl) RequestMembersWithQuery(ctx context.Context, guildID snowflake.ID, query string, limit int) ([]fluxer.Member, error) {
	return m.requestGuildMembers(ctx, guildID, &query, &limit, nil, nil)
}

func (m *memberChunkingManagerImpl) RequestAllMembers(ctx context.Context, guildID snowflake.ID) ([]fluxer.Member, error) {
	query := ""
	limit := 0
	return m.requestGuildMembers(ctx, guildID, &query, &limit, nil, nil)
}

func (m *memberChunkingManagerImpl) RequestMembersWithFilter(ctx context.Context, guildID snowflake.ID, memberFilterFunc func(member fluxer.Member) bool) ([]fluxer.Member, error) {
	query := ""
	limit := 0
	return m.requestGuildMembers(ctx, guildID, &query, &limit, nil, memberFilterFunc)
}

func (m *memberChunkingManagerImpl) RequestMembersChan(guildID snowflake.ID, userIDs ...snowflake.ID) (<-chan fluxer.Member, func(), error) {
	if len(userIDs) == 0 {
...
func (m *memberChunkingManagerImpl) RequestMembersWithQueryChan(guildID snowflake.ID, query string, limit int) (<-chan fluxer.Member, func(), error) {
	return m.requestGuildMembersChan(context.Background(), guildID, &query, &limit, nil, nil)
}

func (m *memberChunkingManagerImpl) RequestAllMembersChan(guildID snowflake.ID) (<-chan fluxer.Member, func(), error) {
	query := ""
	limit := 0
	return m.requestGuildMembersChan(context.Background(), guildID, &query, &limit, nil, nil)
}

func (m *memberChunkingManagerImpl) RequestMembersWithFilterChan(guildID snowflake.ID, memberFilterFunc func(member fluxer.Member) bool) (<-chan fluxer.Member, func(), error) {
	query := ""
	limit := 0
