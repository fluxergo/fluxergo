...
func defaultConfig(gatewayHandlers map[gateway.EventType]GatewayEventHandler) config {
	return config{
		Logger:                 slog.Default(),
		EventManagerConfigOpts: []EventManagerConfigOpt{WithGatewayHandlers(gatewayHandlers)},
...
func BuildClient(
	...
	if token == "" {
		return nil, fluxer.ErrNoBotToken
	}
	...
	cfg := defaultConfig(gatewayHandlers)
	cfg.apply(otps)
	...
	if cfg.Gateway == nil && len(cfg.GatewayConfigOpts) > 0 {
		var gatewayRs *fluxer.GatewayBot
		gatewayRs, err = client.Rest.GetGatewayBot()
		if err != nil {
			return nil, err
		}
