...
import (
	...
	"github.com/fluxergo/fluxergo/webhook"
)
...
func main() {
	...
	// alternatively, you can also construct a webhook client by parsing the webhook URL:
	// client := webhook.NewWithURL(webhookURL)
...
func send(wg *sync.WaitGroup, client *webhook.Client, i int) {
	...
	if _, err := client.CreateMessage(fluxer.NewWebhookMessageCreate().
